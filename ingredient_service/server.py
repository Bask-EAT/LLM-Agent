...
VECTOR_DB_API_URL = os.getenv(
    "VECTOR_DB_API_URL", "https://b2ae656164c1.ngrok-free.app"
)  # 로컬 벡터 DB API 주소
DEFAULT_TOP_K = 10
...
async def search_by_text(request: Request):
    ...
    try:
        ...
        if not query:
            logger.error(
                "=== 💚 [8004 서버] 오류: 요청 본문에 'query' 필드가 없습니다."
            )
        ...
        vector_db_payload = {
            "query": query,
            "top_k": incoming_body.get(
                "top_k", DEFAULT_TOP_K
            ),  # 요청에 top_k가 있으면 사용, 없으면 기본값
        }
        target_url = f"{VECTOR_DB_API_URL}/search/text"

        logger.info(
            f"=== 💚 [8004 서버] 실제 벡터 DB로 요청 전송 시작. URL: {target_url}, Payload: {vector_db_payload}"
        )

        # 3. httpx를 사용해 실제 벡터 DB API를 비동기적으로 호출합니다.
        async with httpx.AsyncClient() as client:
            response = await client.post(
                target_url, json=vector_db_payload, timeout=60.0
            )

            logger.info(
                f"=== 💚 [8004 서버] 실제 벡터 DB로부터 응답 받음. Status: {response.status_code}"
            )
            ...
            logger.info(
                f"=== 💚 [8004 서버] 최종 검색 결과를 planning-agent로 반환합니다."
            )
        ...
        # 기타 예외 처리
        logger.error(
            f"=== ❌ [8004 서버] 처리 중 알 수 없는 오류 발생: {e}", exc_info=True
        )
...
async def search_by_image(
    ...
    try:
        ...
        if not image_bytes:
            logger.error(
                "=== 💚 [8004 서버] 오류: 요청 본문에 'image_bytes' 필드가 없습니다."
            )
            raise HTTPException(
                status_code=400, detail="'image_bytes' 필드가 필요합니다."
            )

        logger.info(
            f"=== 💚 [8004 서버] Base64 이미지 데이터 디코딩 완료 (길이: {len(image_bytes)}) 💚 ==="
        )
        ...
        logger.info(
            f"=== 💚 [8004 서버] 실제 벡터 DB로 요청 전송 시작. URL: {target_url}"
        )

        # 3. httpx를 사용해 실제 벡터 DB API를 비동기적으로 호출합니다.
        async with httpx.AsyncClient() as client:
            response = await client.post(
                target_url, files=files, params=params, timeout=30.0
            )
            ...
            logger.info(
                "=== 💚 [8004 서버] 최종 검색 결과를 planning-agent로 반환합니다."
            )
...
async def search_by_multimodal(request: Request):
    ...
    if not query_text or not image_data:
        raise HTTPException(
            status_code=400, detail="query_text와 image_data가 모두 필요합니다."
        )
