...
def normalize_ingredient_string(raw: str):
    ...
    m_space = re.match(
        r"^([가-힣A-Za-z\s]+?)\s*(\d+[\./,]?\d*)\s*([가-힣A-Za-z%]+)?$", text
    )
    if m_space:
...
def build_recipe_object(source: str, payload: dict) -> dict:
    ...
    if isinstance(ingredients_raw, list):
        ...
        for ing in ingredients_raw:
            if isinstance(ing, dict) and {"item", "amount", "unit"}.issubset(
                ing.keys()
            ):
                structured.append(
                    {
...
def transcript_node(state: GraphState) -> GraphState:
    ...
    try:
        ...
        logger.debug(
            f"DEBUG: 추출된 스크립트 길이: {len(transcript_text) if transcript_text else 0}"
        )
...
def create_recipe_graph():
    ...
    workflow.add_node(
        "video_analyzer", video_analyzer_node
    )  # 비디오 직접 분석 노드 추가
    ...
    def route_after_transcriber(state: GraphState) -> str:
        return (
            "validator"
            if state.get("transcript") and not state.get("error")
            else "video_analyzer"
        )
    ...
    workflow.add_conditional_edges(
        "video_analyzer", finish_after_video_analyzer, {END: END}
    )
    ...
    try:
        ...
        if "error" in result:
            return {
                ...
                "food_name": (
                    result.get("recipe").food_name
                    if result.get("recipe")
                    else result.get("video_title", "")
                ),
                "ingredients": [],
                "recipe": [],
            }
...
async def extract_recipe_from_youtube(youtube_url: str) -> str:
    ...
    try:
        async with aiohttp.ClientSession() as session:
            payload = {"youtube_url": youtube_url, "message": youtube_url}
            logger.debug("=== 🤍payload for VideoAgent Service: %s", payload)

            logger.info(
                f"=== 🤍VideoAgent Service로 요청 전송: {VIDEO_SERVICE_URL}/process"
            )
            async with session.post(
                f"{VIDEO_SERVICE_URL}/process", json=payload
            ) as response:
                if response.status == 200:
                    ...
                    logger.error(
                        f"🚨 VideoAgent Service 오류 (상태: {response.status}): {error_text}"
                    )
                    return {
        ...
        logger.error(
            f"🚨 [TOOL CRASH] 도구 실행 중 심각한 예외 발생: {e}", exc_info=True
        )
        error_dict = {
