...
async def generate_final_answer(state):
    ...
    try:
        ...
        if isinstance(parsed_output, dict) and (
            "source" in parsed_output or "food_name" in parsed_output
        ):
            # 레시피 데이터가 이미 완성된 경우
            # source가 없으면 추가
...
async def run_agent(input_data: dict):
    ...
    try:
        ...
        if image_bytes:
            ...
            youtube_links_in_latest = re.findall(
                r"https?://(?:www\.)?youtube\.com/watch\?v=[\w-]+", latest_message
            )
            ...
            if len(youtube_links_in_latest) == 1:
                ...
                all_youtube_links = re.findall(
                    r"https?://(?:www\.)?youtube\.com/watch\?v=[\w-]+", user_message
                )
