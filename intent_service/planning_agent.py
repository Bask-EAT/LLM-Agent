...
tool_calling_prompt = ChatPromptTemplate.from_messages(
    ...
    ---
    ### **1단계: 사용자 의도 분석 및 도구 결정 (우선순위 순)**

    **🔥 최우선 규칙: 상품 검색 vs 레시피 요청 구분**
    - **상품 검색 우선**: '찾아줘', '찾아', '검색', '어디서', '어디에', '어디', '위치', '구매', '사야해', '가격' 등의 키워드가 포함된 경우 → **반드시** `search_ingredient_by_text` 도구를 호출하세요.
    - **레시피 요청**: '레시피', '만드는 법', '조리법', '요리', '만들어', '어떻게' 등의 키워드가 포함된 경우 → `text_based_cooking_assistant` 도구를 호출하세요.
    ...
    **🛒 상품 검색/구매 요청 확인**
    - 다음 키워드가 포함된 경우 `search_ingredient_by_text` 도구를 호출하세요:
      * '찾아줘', '찾아', '검색', '어디서', '어디에', '어디', '위치'
      * '구매', '사야해', '어디서 사', '가격', '장바구니에 담아', '주문', '배송'
      * '상품 정보', '상품 검색', '상품 찾기', '재료', '식재료'
      * **특정 식재료명 + 찾아줘**: 예) "바나나 찾아줘", "사과 찾아줘", "우유 찾아줘"
      * **식재료명만 단독으로 언급**: 예) "바나나", "사과", "우유" (레시피 요청이 아닌 경우)
    
    **🔥 구체적인 예시:**
    - **상품 검색으로 인식해야 하는 경우:**
      * "바나나 찾아줘" → `search_ingredient_by_text`
      * "사과 어디서 사야해" → `search_ingredient_by_text`
      * "우유 가격 얼마야" → `search_ingredient_by_text`
      * "계란 장바구니에 담아줘" → `search_ingredient_by_text`
      * "소금 구매하고 싶어" → `search_ingredient_by_text`
    
    - **레시피 요청으로 인식해야 하는 경우:**
      * "바나나 레시피" → `text_based_cooking_assistant`
      * "사과로 만드는 요리" → `text_based_cooking_assistant`
      * "우유로 만드는 음식" → `text_based_cooking_assistant`
    
    **🍳 기본값: 요리 관련 대화**
    - 위의 모든 조건에 해당하지 않는 경우 → `text_based_cooking_assistant` 도구를 호출하세요.
    ...
    **⚠️ 주의사항:**
    - "김치전 레시피", "된장찌개 만드는 법" 등은 **절대** 상품 검색이 아닌 **레시피 요청**입니다.
    - 레시피와 상품 검색이 혼재된 경우, **레시피가 우선**입니다.
    """,
        ),
...
json_generation_prompt = ChatPromptTemplate.from_messages(
    ...
    **🚨 매우 중요한 규칙:**
    - `ingredients` 배열이 비어있거나 `recipe` 배열이 비어있으면 → `chatType`은 "chat"입니다!
    - 사용자에게 선택지를 제시하는 경우 → `chatType`은 "chat"입니다!
    - 휴먼 루프가 필요한 경우 → `chatType`은 "chat"입니다!
    - **chat 타입일 때는 화면 변화가 없어야 합니다!**
    ...
    #### **1. `chatType` 결정 (🔥 매우 중요 - 반드시 이 규칙을 따르세요!):**
    ...
    **🔥 우선순위 순서로 판단하세요:**
    ...
    1. **`chatType`이 "cart"인 경우:**
       - `tool_output`에 'product_name'과 'price'가 포함되어 있으면 "cart"
       - `tool_output`에 'products' 또는 '상품' 관련 정보가 포함되어 있으면 "cart"
       - `tool_output`에 'search_ingredient_by_text' 또는 'search_ingredient_by_image' 호출 결과가 포함되어 있으면 "cart"
    
    2. **`chatType`이 "recipe"인 경우:**
       - `tool_output`에 'recipe' 배열이 있고, 그 안에 **실제 요리법 단계가 포함되어 있어야 함**
       - `tool_output`에 'ingredients' 배열이 있고, 그 안에 **실제 재료 정보가 포함되어 있어야 함**
       - **🚨 절대 규칙**: `ingredients=[]` 또는 `recipe=[]`이면 "chat"으로 분류!
    
    3. **`chatType`이 "chat"인 경우 (기본값):**
       - 위의 두 조건에 해당하지 않는 **모든 경우**
       - 사용자에게 질문하거나 선택지를 제시하는 경우
       - 레시피나 상품 정보가 없는 일반적인 대화
       - 휴먼 루프가 필요한 경우
       - **🚨 절대 규칙**: `ingredients=[]` 또는 `recipe=[]`이면 "chat"!
       - **chat 타입일 때는 화면 변화가 없어야 합니다!**
    
    **📋 구체적인 예시:**
    - **"chat"으로 분류해야 하는 경우:**
      * `ingredients: []` + `recipe: []` → "chat"
      * "어떤 볶음밥을 원하시나요?" → "chat" (선택지 제시)
      * "1. 김치볶음밥 2. 새우볶음밥" → "chat" (선택지 제시)
      * 휴먼 루프가 필요한 경우 → "chat"
    
    - **"recipe"로 분류해야 하는 경우:**
      * `ingredients: [{"item": "밥", "amount": "1", "unit": "공기"}]` + `recipe: ["1단계: 밥을 준비합니다"]` → "recipe"
    ...
    - **`chatType`이 "cart"일 경우:**
      - `tool_output`의 `content` 또는 `answer`를 참고하여 친절한 답변을 생성하세요.
      - **[핵심 규칙]** `tool_output`에서 `ingredients` 배열의 각 상품 정보를 그대로 사용하세요.
      - `tool_output`에 `recipes` 배열이 있다면 그대로 사용하고, 없다면 빈 배열로 설정하세요.
      - 최종 구조는 반드시 아래와 같아야 합니다.
      ```json
      {{
        "chatType": "cart",
    ...
    - **`chatType`이 "chat"일 경우:**
      ...
      - 레시피나 상품 정보가 없는 일반적인 대화
      - 휴먼 루프가 필요한 경우
      - 레시피 내용이 비어있거나 사용자에게 선택지를 제시하는 경우
      - **chat 타입일 때는 화면 변화가 없어야 합니다!**
    ```json
      {{
        "chatType": "chat",
    ...
    **🚨 최종 확인 사항:**
    - `ingredients=[]` 또는 `recipe=[]`이면 반드시 `chatType: "chat"`으로 설정하세요!
    - 사용자에게 선택지를 제시하는 경우 반드시 `chatType: "chat"`으로 설정하세요!
    - 휴먼 루프가 필요한 경우 반드시 `chatType: "chat"`으로 설정하세요!
    - 실제 레시피 내용이 있을 때만 `chatType: "recipe"`로 설정하세요!
    - **chat 타입일 때는 화면 변화가 없어야 합니다!**
    """,
        ),
        MessagesPlaceholder(variable_name="messages"),
...
async def select_tool(state):
    ...
    try:
        logger.info(
            "--- [LangGraph] ⏳ agent_runnable.ainvoke 호출 시작... (이 단계에서 외부 API 호출로 인해 시간이 소요될 수 있습니다) ---"
        )
        ...
        logger.error(
            f"--- [LangGraph] 🚨 agent_runnable.ainvoke 호출 중 심각한 오류 발생! 🚨 ---"
        )
...
async def custom_tool_node(state: AgentState, config: RunnableConfig):
    ...
    logger.info(
        f"--- [LangGraph] 🛠️ Tool 실행 완료. 결과: {tool_output_str}"
    )  # ★★★ 벡터DB에서 가져온 최종 검색 결과임 ★★★
...
final_prompt_for_formatter = ChatPromptTemplate.from_template(
    ...
    #### 1. `chatType` 결정:
    - **🚨 절대 규칙**: `ingredients=[]` 또는 `recipe=[]`이면 반드시 `chatType: "chat"`으로 설정하세요!
    - **🚨 절대 규칙**: 사용자에게 선택지를 제시하는 경우 반드시 `chatType: "chat"`으로 설정하세요!
    - **🚨 절대 규칙**: 휴먼 루프가 필요한 경우 반드시 `chatType: "chat"`으로 설정하세요!
    - **chat 타입일 때는 화면 변화가 없어야 합니다!**
    
    - Tool Output에 레시피 정보(예: 'recipe', 'ingredients' 키)가 있고, 실제 내용이 있다면 'recipe'로 설정하세요.
    ...
    - 이 리스트를 최종 JSON의 `recipes[0].ingredients` 키의 값으로 사용하세요.
...
async def generate_final_answer(state):
    ...
    logger.info(
        f"--- [LangGraph] ✍️ 최종 응답: {final_response_msg} ---"
    )  # ★★★ formatter_chain에서 tool_output(원본 데이터)를 가공한 최종 응답
...
async def run_agent(input_data: dict):
    ...
    try:
        ...
        if chat_history and len(chat_history) > 0:
            logger.info(
                f"--- [STEP 1a] 채팅 히스토리 처리: {len(chat_history)}개 메시지 ---"
            )
            ...
            logger.info(f"--- [STEP 1b] 변환된 메시지: {len(messages)}개 ---")
            ...
            if image_bytes:
                ...
                ]  # user_message가 None일 경우 빈 문자열로 처리
                inputs = {
                    "messages": messages,
                    "image_b64": None,
                }  # image_b64는 None으로 전달
        ...
        if not output_string:
            ...
            error_response = {
                "chatType": "error",
                "answer": "죄송합니다, 답변을 생성하는 데 실패했습니다. 응답이 비어있습니다.",
            }

            # 백엔드 ChatResponse 모델에 맞는 에러 응답 생성
        ...
        if match:
            ...
            logger.info(
                "--- [STEP 6a] 마크다운 블록에서 JSON을 성공적으로 추출했습니다. ---"
            )
        else:
            # 만약 ```json ``` 마크다운을 생성하지 않을 시 전체 문자열 사용 (LLM이 지시를 완전히 따르지 않은 경우일 수 있음)
            logger.warning(
                "--- [STEP 6b] JSON 블록을 찾지 못했습니다. 전체 문자열을 사용합니다. ---"
            )
            clean_json_string = output_string.strip()

        # --- 디버깅 코드 추가 ---
        logger.info(
            "--- [STEP 7] 파싱할 최종 JSON 문자열(Cleaned JSON)을 준비했습니다. ---"
        )
        logger.info(
            f"\n<<<<<<<<<< CLEAN JSON START >>>>>>>>>>\n{clean_json_string}\n<<<<<<<<<<< CLEAN JSON END >>>>>>>>>>>"
        )
        ...
        try:
            ...
            logger.info(
                f"--- [STEP 9] json.loads()가 정상적으로 완료되었습니다. 데이터 타입: {type(parsed_data)} ---"
            )  # 데이터 타입: <class 'dict'> ---
        except json.JSONDecodeError as json_error:
            logger.error(f"--- [ERROR] JSON 파싱 실패: {json_error}")
            # JSON 파싱 실패 시 에러 응답 생성
            error_response = {
                "chatType": "error",
                "answer": f"JSON 파싱에 실패했습니다: {json_error}",
            }
        ...
        logger.info(
            "--- ✅ [마지막 단계] 모든 처리가 완료되었습니다. 이제 파싱된 딕셔너리를 반환합니다. ---"
        )

        # 백엔드 ChatResponse 모델에 맞는 형태로 변환
        # 기본 응답 구조 생성
        backend_response = {
            ...
            "payload": parsed_data,  # LLM의 원본 응답을 payload에 포함
        }

        logger.info(
            "--- ✅ [백엔드 응답 변환] ChatResponse 모델에 맞는 형태로 변환 완료 ---"
        )
        return backend_response  # ---→ 🤍❤ Agent 최종 응답 로그 부분
